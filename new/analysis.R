...
W = data$pp_dummy
cluster = data$id
data$omega <- data$omegahat_tl
covariates <- c("cogs", "k", "year", "nace2")
X = data[, covariates]
str(X)
...
average_treatment_effect(cf.full, target.sample = "overlap")
...
ate.by.quartile = lapply(samples.by.quartile, function(samples) {
  average_treatment_effect(eval.forest, subset = samples, target.sample = "overlap")
})
...
Y.forest = regression_forest(X[train, ], Y[train], num.trees = 500, clusters = cluster[train])
Y.hat = predict(Y.forest)$predictions

# Estimate e(x) = E[W | X = x] using a regression forest.
W.forest = regression_forest(X[train, ], W[train], num.trees = 500, clusters = cluster[train])
...
selected.vars = which(varimp.Y >= quantile(varimp.Y, 0.6))
