...
library(readr)
data <- read_csv("markups.csv")
str(data)
n <- nrow(data)

# Treatment: does the the gov't spend too much on "welfare" (1) or "assistance to the poor" (0)
data$w <- data$pp_dummy
treatment <- "w"

# Outcome: 1 for 'yes', 0 for 'no'
data$Y <- log(data$markup)
outcome <- "Y"

data$omega <- data$omegahat_tl
# Additional covariates
covariates <- c("k", "cogs", "year", "nace2", "omega")
...
data$nace2 <- as.factor(data$nace2)
...
cluster = data$id
data$omega <- data$omegahat_tl
covariates <- c("cogs", "k", "year", "nace2", "omega")
X = data[, covariates]
...
ct.unpruned <- honest.causalTree(
  ...
  treatment=t(data[indices$split, treatment]),
  est_data=data[indices$est,],
  est_treatment=t(data[indices$est, treatment]),
...
```{r, cov-value-leaf, fig.align = 'center', fig.cap= "Average covariate values within each leaf", fig.width=10}
...
```{r avg-cov-cate, fig.align = 'center', fig.cap= "Average covariate values within group (based on CATE estimate ranking)"}
df <- mapply(function(covariate) {
      # Looping over covariate names
