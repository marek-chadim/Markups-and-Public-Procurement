...
We begin with an empirical example from Hainmueller and Hopkins (2019), in which the authors investigate the effects of D democracy (versus direct democracy) on naturalization rates in Switzerland using municipality-year level panel data from 1991 to 2009. The study shows that a switch from direct to D democracy increased naturalization rates by an average of 1.22 percentage points (Model 1 of Table 1 in the paper).
...
First, we will examine the evolution of treatment status using the **panelView** package. The variables `id` and `year` are the unit and time indicators, respectively. The treatment variable is `D`, and the outcome is `Y`. In the baseline analysis, we will not include any covariates. We will set the option `by.timing = TRUE` to sort units by the timing of receiving the treatment.
...
ggplot(df_bacon) +
   ...
   geom_point()
print(aggregate(df_bacon$weight, list(df_bacon$type), FUN=sum))
```
...
df <- as.data.frame(data %>% group_by(id) %>% mutate(treatment_mean = mean(D,na.rm = TRUE)))
df.use <- df[which(df$treatment_mean<1),]

# Re-estimate TWFE on this Sub-sample
model.twfe.1 <- feols(Y~D|id+year,
                      data=df.use, cluster = "id")
print(model.twfe.1)
```
...
df.use <- get.cohort(df.use,D="D",index=c("id","year"),start0 = TRUE)
head(df.use[,-5],19)
```
...
twfe.est <- feols(Y ~ i(Time_to_Treatment, treat, ref = -1)| id + year,  
                  data = df.twfe, cluster = "id")
...
twfe.output$Time <- c(c(-12:-2),c(0:14)) 
p.twfe <- esplot(twfe.output,Period = 'Time',Estimate = 'Estimate',
...
df.st$st_unit <- as.numeric(factor(paste0(df.st$stack,'-',df.st$id)))
df.st$st_year <- as.numeric(factor(paste0(df.st$stack,'-',df.st$year)))
model.st <- feols(Y~D|st_unit+st_year,
...
st.est <- feols(Y ~ i(Time_to_Treatment, treat, ref = -1)| st_unit + st_year,data = df.st,cluster = "st_unit")
...
st.output$Time <- c(c(-14:-2),c(0:14))+1 
...
model.sa.1 <- feols(Y~sunab(FirstTreat,year)|id+year,
                    data = df.sa, cluster = "id")
...
sa.output$Time <- c(c(-14:-2),c(0:14))+1
...
pm.output <- cbind.data.frame(ATT=coef, se=sd, t=c(-3:0))
...
esplot(data=to_plot,Period = "term",Estimate = 'estimate',SE = 'std.error',xlim = c(-11,15))
...
out.fect.balance <- fect(Y~D, data = df, index = c("id","year"),
                         method = 'fe', se = TRUE, balance.period = c(-3,1))
#ATT
...
index.use <- which(rownames(out.fect$est.att) %in% as.character(c(-2:3)))
beta.hat <- out.fect$est.att[index.use,1]
vcov.hat <- out.fect$att.vcov[index.use,index.use]
count <- out.fect$count[which(rownames(out.fect$est.att) %in% as.character(c(1:3)))]
...
ggsave("sensitivity_plot.png", width = 7, height = 5)
```
...
p.fect.honest <- esplot(fect.output,Period = 'Time',Estimate = 'ATT',
                   ...
                   CI.upper = 'CI.upper',xlim = c(-5,5),ylim = c(-.05,.35),Count = 'count',show.count = T)

p.fect.honest <- p.fect.honest + geom_linerange(aes(x=post,ymin=lb,ymax=ub),data = dte_output[which(dte_output$Mbar==0),],color = 'maroon',size=1.2)

p.fect.honest
```

```{r hh_honest_fect, message = FALSE, warning = FALSE, fig.width = 7, fig.height = 5, cache=TRUE}
ggsave("sensitivity_plot2.png", width = 7, height = 5)
```
...
out.fect.placebo <- fect(Y~D, data = data, index = c("id","year"),
                         method = 'fe', se = TRUE, 
...
```{r hh_honest_fect, message = FALSE, warning = FALSE, fig.width = 7, fig.height = 5, cache=TRUE}
ggsave("sensitivity_plot3.png", width = 7, height = 5)
```
...
p.placebo.honest <- esplot(fect.output,Period = 'Time',Estimate = 'ATT',
                   SE = 'S.E.',CI.lower = "CI.lower", main = "FEct",ylab = "Coefficients and 95% CI",
                   CI.upper = 'CI.upper',xlim = c(-5,3),ylim = c(-.1,.3),Count = 'count',show.count = T)
dte_output <- as.data.frame(dte_output)

p.placebo.honest <- p.placebo.honest + geom_linerange(aes(x=post,ymin=lb,ymax=ub),data = dte_output[which(dte_output$Mbar==3),],color = 'maroon1',linewidth = 1)
...
```{r hh_honest_fect, message = FALSE, warning = FALSE, fig.width = 7, fig.height = 5, cache=TRUE}
ggsave("sensitivity_plot4.png", width = 7, height = 5)
```
...
panelview(Y=y, D=d, X=controls, index = index, data = data, xlab = "Time Period", ylab = "Unit", display.all = TRUE,
...
twfe.est <- feols(Y ~ i(Time_to_Treatment, ref = -1) | id + year,  
...
df.pm[,"id"] <- as.integer(as.factor(df.pm[,"id"]))
df.pm[,"year"] <- as.integer(as.factor(df.pm[,"year"]))
df.pm <- df.pm[,c("Y", "pp_dummy", "id","year")]
...
model.fect <- fect(Y = y, D = d, X= controls, data = data, 
                   method = "mc", CV=TRUE, nlambda=50, 
                   index = index, se = TRUE, parallel = TRUE, seed = 42)
...
fect.output <- as.data.frame(fect.output)
fect.output$Time <- c(-5:2)
p.fect <- esplot(fect.output,Period = 'Time',Estimate = 'ATT', xlim=c(-5,2),
                   SE = 'S.E.',CI.lower = "CI.lower", 
                   CI.upper = 'CI.upper')
p.fect
```
...
out.fect.p <- fect(Y = y, X = controls, D = d, data = data, index = index,
                   method = 'ife', se = TRUE, placeboTest = TRUE, placebo.period = c(-2,0))
