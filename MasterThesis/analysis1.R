...
model.twfe <- feols(Y ~ pp_dummy |id + year,
...
twfe.est <- feols(Y ~ i(Time_to_Treatment, treat, ref = -1) + 
                    pp_dummy | id + year,  
...
df.pm <- df.pm[,c("id","year","pp_dummy","Y")]
...
model.fect <- fect(Y = y, D = d, data = data, 
                   method = "fe", 
