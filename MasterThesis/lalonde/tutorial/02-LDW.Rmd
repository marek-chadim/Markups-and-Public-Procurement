...
Y <- "ymu1"
treat <- "pp_dummy"
covar <- c("lpp", "x2", "pp_history","year", "x1", "sales", "costs", "assets", "wages", "empl", "nace2", "legal", "sector")
```
...
```{r, class.source = "fold-show", fig.cap='FIGURE1. SubfigureA:LDW-Experimental.', out.width='80%', fig.asp=1, fig.align='center'}
#| code-fold: show
d_ps <- assess_overlap(data = d, treat = treat, cov = covar)
```
...
d_trim <- trim(d_ps, threshold = 0.9)
d_trim2 <- trim(d_ps, threshold = 0.8)
```
...
ldw_cps_ps <- assess_overlap(data = d, treat = treat, cov = covar)
...
d_trim_match <- psmatch(data = d_trim, Y = "y1", treat = "treat", cov = covar)

d_trim2_match <- psmatch(data = d_trim2, Y = "y1", treat = "treat", cov = covar)
...
d_trim_match_ps <- assess_overlap(data = d_trim_match, treat = treat, cov = covar, xlim = c(-3,3))


d_trim2_match_ps <- assess_overlap(data = d_trim2_match, treat = treat, cov = covar, xlim = c(-3,3))
```
...
#| layout-ncol: 2
# cps data
love.plot(d, d_trim_match, treat = treat, covar = covar, title = "Covariate Balance trim")

# psid data
love.plot(d, d_trim2_match, treat = treat, covar = covar, title = "Covariate Balance trim2")
```
...
Y <- "x2"
treat <- "pp_dummy"
covar <- c("pp_history","year", "x1", "sales", "costs", "assets", "wages", "empl", "nace2", "legal", "sector")

out1 <- estimate_all(d_trim, Y = "x2", treat = "treat", cov = covar)
out2 <- estimate_all(d_trim2,  Y = "x2", "treat", covar)
```
...
est <- out2[1, 1]
plot_coef(out2, line = est, ylim = c(-.5, .5))
