...
s_ps <- assess_overlap(data = d, treat = treat, cov = covar, ylim = c(-0.4, 0.4), breaks = 30, seed = 1234)
```

```{r}
trim <- function(data, ps = "ps_assoverlap" ,threshold_upper = .8) {
  sub <- data[which(data[, ps] < threshold_upper), ]
  return(sub)
}
s_ps_match <- psmatch(data =trim(s_ps), Y = "y1", treat = treat, cov = covar, seed = 1234)

```

```{r}
love.plot(d, s_ps_match, treat = treat, covar = covar, title = "Covariate Balance Sample ")

```

```{r}
s_ps2 <- assess_overlap(data = s_ps_match, treat = treat, cov = covar, ylim = c(-0.4, 0.4), breaks = 30, seed = 1234)
```
...
```{r}
par(mfrow = c(1,2))
s_ps <-assess_overlap(data = d, treat = treat, cov = covar, odds=FALSE, breaks = 30, xlim = c(0, 1), ylim = c(-0.2, 0.2))

s_ps2 <-assess_overlap(data = s_ps_match, treat = treat, cov = covar,odds=FALSE,  breaks = 30, xlim = c(0, 1), ylim = c(-0.2, 0.2))
```
...
Trimming does improve the overlap between the treated and the control, and I will proceed with the trimmed dataset.
...
par(mfrow = c(2,1))
ylim <- c(-.05, .225)
plot_coef(out1, ylim = ylim, main = "Matched Sample (97 : 97)", main.pos = 3)
plot_coef(out2, ylim = ylim, main = "Matched Sample  (97 : 97): Placebo", main.pos = 3)
...
par(mfrow = c(1,1))
plot(1, xlim = c(3.7, 9.3), ylim = c(-.1, .225), type = "n", axes = FALSE, 
    ylab = "Effects on Markups (percentage)", xlab = "Year Relative to Contract")
box(); axis(2)
axis(1, at = 4:9, labels = c(-3:3))
...
legend("topright", legend = c("DIM,  Full (123: 137)", "AIPW, Full (123: 137)", 
    "AIPW, PS Matched (96: 96)"), lwd = 2,
