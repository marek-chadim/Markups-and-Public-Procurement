...
names(df)
# Controls
x      <- "cogs + k + nace2 + year" # use this for tree-based methods like forests and boosted trees
xl     <- "(poly(cogs, 3, raw=TRUE) + poly(k, 3, raw=TRUE) + poly(year, 3, raw=TRUE) + nace2 + empl)^2";  # use this for rlasso etc.
...
r <- foreach(k = 1:split, .combine='rbind', .inorder=FALSE, .packages=vec.pac) %dopar% { 
  ...
  dml <- DoubleML(data=df, y=y, d=d, z=NULL, xx=x, xL=xl, methods=methods, DML="DML1", nfold=2, est="plinear", args=args,  silent=FALSE, trim=c(0.01,0.99), param=param, tune_param=tune_param) 
...
result[1,]        <- colQuantiles(as.matrix(r[,1:(length(methods)+1)]), probs=0.5)
result[2,]        <- colQuantiles(as.matrix(sqrt(r[,(length(methods)+2):ncol(r)]^2+(t(t(r[,1:(length(methods)+1)])) - colQuantiles(as.matrix(r[,1:(length(methods)+1)]), probs=0.5))^2)), probs=0.5)
result[3,]        <- colQuantiles(as.matrix(r[,(length(methods)+2):ncol(r)]), probs=0.5)
